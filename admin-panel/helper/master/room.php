...
elseif ($operation=="updateRoom") {
	...
	if(!empty($_FILES['timage']['name'])){
		...
		if (!empty($room_det->room_thumb) AND file_exists("../../../files/property-logo/".$room_det->roomcat_thumb)){
			@unlink("../../../files/property-logo/".$room_det->roomcat_thumb);
		}
	}
	...
	if(!empty($_FILES['image']['name'])){
		...
		if (!empty($room_det->room_coverimg) AND file_exists("../../../files/property-banner/".$room_det->roomcat_coverimg)){
			@unlink("../../../files/property-banner/".$room_det->roomcat_coverimg);
		}
	}
...
elseif ($operation=="activeroom" OR $operation=="deactiveroom" OR $operation=="deleteroom") {
	...
	// 	$sql = "UPDATE  room_list SET room_status = 2 , room_delete_at= NOW() WHERE room_roomcat_id_ref = '$id'";
	// 	$insert = $PDO->prepare($sql);
	// 	$insert->execute();
	// }
	$ad  = ($operation=='deleteroom')?", roomcat_delete_at=NOW()":null;
