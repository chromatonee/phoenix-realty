...
if ($operation=="fetch"){
	...
	if((!empty($proj_img)) AND file_exists("../../../files/team/".$proj_img)) {
		echo '<img src="../files/team/'.$proj_img.'" height="20">';
	}else{
		echo '<img src="../files/team/user.png" height="20">';
	}
	?>		
	</td>
	...
	<?php }else{echo '<div class="alert alert-warning"><p>No Members Found</p></div>'; }
}
...
elseif($operation=="update") {
	...
	if(!empty($_FILES['image']['name'])){
		...
		$img_file  = resize($width,$height,$dir,$img_thumb);
		if ((!empty($chk_id->proj_img)) AND file_exists("../../../files/team/".$chk_id->proj_img)) {
			@unlink("../../../files/team/".$chk_id->proj_img);
		}
	}
