...
if ($operation=="fetch"){
	...
	if((!empty($abo_img)) AND file_exists("../../../files/workwith/".$abo_img)) {
		echo '<img src="../files/workwith/'.$abo_img.'" height="20">';
	}else{
		echo '<img src="../files/workwith/user.png" height="20">';
	}
	?>		
	</td>
...
elseif($operation=="update") {
	...
	if(empty($uptid) OR empty($upname)){
		echo $response = json_encode(array(
				"status" => false,
				"msg"	 => "Fields is Empty"
		));
	...
	if(!empty($_FILES['image']['name'])){
		...
		$img_file  = resize($width,$height,$dir,$img_thumb);
		if ((!empty($chk_id->abo_img)) AND file_exists("../../../files/workwith/".$chk_id->abo_img)) {
			@unlink("../../../files/workwith/".$chk_id->abo_img);
		}
	}
